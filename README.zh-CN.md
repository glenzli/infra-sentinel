...
系统 Collector 消费平台无关的能力合同，不假设每个系统都能提供相同计数。当前经过验证的 macOS backend 会观测整机 CPU 使用率、原生内存压力、压缩内存与 Swap、磁盘剩余空间、物理磁盘字节与操作次数、保守的磁盘健康证据、温度压力。当前值跟随 Agent 正常采样刷新，但活动中的 15 分钟桶只保留在内存；桶结束后再用一次事务批量落盘。平台支持时，磁盘健康在启动时读取一次，之后每 6 小时刷新。
