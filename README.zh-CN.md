...
整机与 App 当前值跟随 Agent 正常采样刷新，但活动中的 15 分钟桶只保留在内存；桶结束后再用一次事务批量落盘，较旧历史继续按现有规则压缩为小时和每日粒度。进程归因可能遗漏短命进程或无权读取的系统服务，也不应预期与物理设备计数完全相等。平台支持时，磁盘健康在启动时读取一次，之后每 6 小时刷新。
...
告警只采用可靠的压力信号：macOS 内存压力、磁盘空间低于 10% 或 5%，以及较高或严重温度压力。CPU 和磁盘高活动会展示趋势，但在没有持续压力合同前不会仅凭数值触发告警。App 归因只作为诊断证据，不等同于审计口径或按 Agent 的计费边界。
...
其中包括 SQLite 指标库、低频计数器 checkpoint、健康状态、临时命令结果、滚动日志和受限保留的旧网络 JSONL。最近 7 天保留 15 分钟粒度，之后到第 90 天压缩为小时粒度，更早历史压缩为每日粒度；计数器总量保持可加，状态值保留加权平均、最小、最大与最近值。有效命令结果在桌面端读取后立即删除，无人读取的结果也会自动过期。状态可能包含聚合域名、Xray 客户端标签、模型名和用户设置的显示名称，但不会保存：
