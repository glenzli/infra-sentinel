...
def create_system_backend() -> SystemResourceBackend:
    if sys.platform == "darwin":
        from system_resources_macos import MacOSSystemBackend
        return MacOSSystemBackend()
    if sys.platform.startswith("linux"):
        from system_resources_linux import LinuxSystemBackend
        return LinuxSystemBackend()
    if sys.platform == "win32":
        from system_resources_windows import WindowsSystemBackend
        return WindowsSystemBackend()
    raise RuntimeError(f"system resource collection is unsupported on {sys.platform}")
