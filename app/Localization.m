...
static NSDictionary<NSString *, NSString *> *ChineseStrings(void) {
    ...
    dispatch_once(&onceToken, ^{
        strings = @{
            ...
            @"xray.factor_valid_format": @"校验 %.2f×",
            @"xray.factor_incomplete_format": @"覆盖缺口 %.2f×",
            @"xray.factor_waiting": @"等待校验",
            ...
            @"estimate.comparable_scope_format": @" · 单主机计量校验",
            @"estimate.empirical_waiting": @"等待 VPS 与 Xray 的完整对齐区间",
            @"estimate.per_vps_only_format": @"已配置 %ld 台主机：倍率按单台链路分别校验",
            @"estimate.per_vps_only_detail": @"不同主机的网卡覆盖与路由可能不同；总账单可以汇总，倍率不可跨主机合并。",
            @"estimate.incomplete_coverage_format": @"链路覆盖不完整：观测 %.2f×，低于基准 %.2f×",
            @"estimate.incomplete_coverage_detail": @"所选网卡未覆盖完整流量路径，或 Xray 用户计数范围不同；不将差额解释为协议开销。",
            ...
            @"settings.remote_fixed": @"固定使用自动网卡、5 分钟低频读取；预算按本主机的计费周期与计费方向计算。",
...
static NSDictionary<NSString *, NSString *> *EnglishStrings(void) {
    ...
    dispatch_once(&onceToken, ^{
        strings = @{
            ...
            @"xray.factor_valid_format": @"verified %.2f×",
            @"xray.factor_incomplete_format": @"coverage gap %.2f×",
            @"xray.factor_waiting": @"waiting",
            ...
            @"remote.title": @"Host measurement details",
            @"remote.server_format": @"%@  ·  %@",
            @"estimate.empirical_format": @"Measured billing %.2f× · two-leg baseline %.2f× · bill uplift +%.1f%%",
            @"estimate.comparable_scope_format": @" · single-host measurement",
            @"estimate.empirical_waiting": @"Waiting for a complete aligned VPS and Xray interval",
            @"estimate.per_vps_only_format": @"%ld hosts configured: factors are verified per route",
            @"estimate.per_vps_only_detail": @"Host interface coverage and routing can differ; billing totals aggregate, but factors do not cross hosts.",
            @"estimate.incomplete_coverage_format": @"Incomplete route coverage: observed %.2f× below %.2f× baseline",
            @"estimate.incomplete_coverage_detail": @"The selected interface misses part of the route, or Xray user coverage differs; the difference is not treated as protocol overhead.",
            ...
            @"notice.reset_requested": @"Reset request sent to the local Agent. New baselines start with the next sample interval.",
            ...
            @"settings.subtitle": @"Local network discovery needs no setup. Alerts and remote host measurement are configurable.",
            @"settings.monitor_section": @"Alerts",
            @"settings.warning": @"Warning (one way)",
            @"settings.critical": @"Critical (combined)",
            @"settings.minutes": @"min",
            @"settings.mib": @"MiB",
            @"settings.remote_section": @"Hosts",
            @"settings.server_add": @"Add VPS",
            @"settings.server_header": @"Name / SSH Host / Xray / Billing",
            @"settings.server_label_placeholder": @"Name",
            @"settings.xray_enable_short": @"Xray",
            @"settings.billing_budget_short": @"Budget",
            @"settings.billing_cycle_short": @"Cycle",
            @"settings.day_short": @"day",
            @"settings.billing_warning_short": @"Warning",
            @"settings.billing_critical_short": @"Critical",
            @"settings.remote_enable": @"Enable Linux VPS network measurement",
