...
function resourceCard(resource: ResourceProjection): string {
  ...
  const sourceSummary = `${resource.online_source_count} / ${resource.source_count} ${tr("sources online", "个数据源在线")}`;
  return `<${tag} class="resource-card resource-card--${escapeHtml(resource.status)}"${type}${interaction}><div class="resource-card__heading"><span class="resource-card__identity"><span class="resource-card__state source-state source-state--${escapeHtml(resource.status)}" aria-hidden="true"></span><p>${escapeHtml(resourceLabel(resource))}</p></span><span class="pill pill--${escapeHtml(resource.status)}">${escapeHtml(statusLabel(resource.status))}</span></div><div class="resource-card__metric"><strong>${escapeHtml(resourceValue(resource))}</strong><small>${escapeHtml(sourceSummary)}</small></div>${canOpen ? `<span class="resource-card__open">${tr("Details", "详情")} →</span>` : ""}</${tag}>`;
}
