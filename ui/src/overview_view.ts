...
function aiUsageCard(projection: AgentProjection, resource: ResourceProjection): string {
  ...
  return `<button class="resource-card resource-card--ai-usage resource-card--${escapeHtml(resource.status)}" type="button" data-resource-id="ai_usage"><div class="resource-card__heading"><span class="resource-card__identity"><span class="resource-card__state source-state source-state--${escapeHtml(resource.status)}" aria-hidden="true"></span><p>${tr("AI usage", "AI 用量")}</p></span></div><div class="ai-overview__rollup"><section><small>${tr("Observed today", "今日已观测")}</small><strong>${formatTokens(today.tokens)}</strong>${ranking("today")}</section><section><small>${tr("Local history total", "本机历史总量")}</small><strong>${formatTokens(cumulative.tokens)}</strong>${ranking("cumulative")}</section></div><div class="network-overview__footer ai-overview__footer"><span>${tr("Local rollup · not billing", "本机汇总 · 非账单")}</span><span>${sourceSummary(resource)}</span><span>${tr("Details", "详情")} →</span></div></button>`;
}
